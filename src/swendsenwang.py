...
for j in range(tauq.size):
    ...
    for i in range(REPEAT):
        m2 = MC.quench_swendsenwang(L, TC, tauq[j])
        m20 = np.append(m20, m2)
        print (j, i, m2)
    m2ave = np.append(m2ave, np.average(m20))
    m2std = np.append(m2std, np.std(m20)) 

np.save("SWL48_m2ave.npy", m2ave)
np.save("SWL48_m2std.npy", m2std)
np.save("SWL48_tauq.npy", tauq)
np.save("SWL48_v.npy", v)


plt.figure()
plt.plot(1/v, m2ave*L**2, label="L=48")
plt.legend()
plt.xscale("log")
plt.yscale("log")
plt.show()